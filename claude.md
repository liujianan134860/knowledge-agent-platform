...
# Session Log — 2026-05-19 (续 9)

## 本次操作摘要
切换到 PostgreSQL + pgvector（从 H2 + SimpleVectorStore）。

## 修改清单

### 43. `.env` — 启用 PostgreSQL 连接
- 取消注释 POSTGRES_URL / POSTGRES_DRIVER / POSTGRES_USER / POSTGRES_PASSWORD / HIBERNATE_DIALECT / PGVECTOR_INIT_SCHEMA
- 新增 `H2_CONSOLE_ENABLED=false`（禁用 H2 控制台）
- 默认值：`localhost:5432/knowledge_agent`，用户 `postgres`，密码 `postgres`

### 44. `application.yml` — H2 console 条件化
- `spring.h2.console.enabled` 从硬编码 `true` 改为 `${H2_CONSOLE_ENABLED:true}`
- 默认开启（H2 模式），PostgreSQL 模式下 `.env` 设置为 `false`

### 45. `.env.example` — 同步更新
- PostgreSQL 注释段新增 `H2_CONSOLE_ENABLED=false` 和 pgvector 安装提示

## Flyway 兼容性审计

所有 8 个迁移文件（V1-V8）均为标准 SQL，PostgreSQL 完全兼容：
- `GENERATED BY DEFAULT AS IDENTITY` — PG 10+ 原生支持
- `VARCHAR`、`TEXT`、`TIMESTAMP`、`BOOLEAN`、`DOUBLE PRECISION` — 标准类型
- `CREATE INDEX`、`ALTER TABLE`、`FOREIGN KEY` — 标准 DDL

## 架构说明

```
数据库切换逻辑：
  POSTGRES_URL 已设置  → PostgreSQL + pgvector (PgVectorStore)
  POSTGRES_URL 未设置  → H2 + SimpleVectorStore (默认)

VectorStore 选择：
  PGVECTOR_INIT_SCHEMA=true  → PgVectorStoreConfig 激活
  PGVECTOR_INIT_SCHEMA=false → SimpleVectorStoreConfig 激活
  未设置                      → SimpleVectorStoreConfig (matchIfMissing)
```

## PostgreSQL 本地安装步骤

```bash
# Windows: 下载安装 PostgreSQL 15+ (https://www.postgresql.org/download/)
# 安装时记住 postgres 用户密码
...
# 3. 启动应用（Flyway 自动创建表，PgVectorStore 自动创建向量表）
mvn spring-boot:run
```
